...
-   `type`：一个字符串字面量，用于声明块的性质。例如，`"markdown"`表示一个散文块，`"code"`表示一个代码块，`"link"`表示一个链接块。渲染层将根据此类型来决定如何展示和处理该块。
...
## 2.4. Link Block：跨文档引用的基础设施

### 2.4.1. Link Block的设计原理

Link Block是一种特殊类型的区块（`type: "link"`），专门用于表示对其他区块的引用关系。它是构建跨文档DAG结构的核心基础设施，使得`.elf`文档能够形成一个分布式的、互相关联的知识图谱。

### 2.4.2. Link Block的数据结构

一个标准的Link Block包含以下特殊属性：
...
```json
{
  ...
  "content": {
    ...
    "ref_type": "include", // "include", "reference", "embed"
    "display_text": "共享工具函数",
...
Link Block使用标准化的URI格式来指向目标：

- **完整格式**：`elf://[user/]repo/doc[#block-name]`
- **相对引用**：`./doc#block-name`（同仓库）或 `#block-name`（同文档）
- **示例**：
  - `elf://alice/research/paper-2024#methodology`
  - `elf://shared-lib/utils/helpers#string-utils`
  - `./components#shared-header`

### 2.4.4. 扩展DAG结构

通过Link Block，`.elf`文档的结构从简单的块列表扩展为完整的DAG：

- **节点**：所有类型的区块（包括Link Block）
- **边**：
  - `parent`关系：构建文档内层级结构
  - `link`关系：构建跨文档引用网络
- **图特性**：有向、可能包含跨文档边、禁止循环引用
...
-   **`type: "link"`**：Link Block的冲突解决需要特别小心，因为错误的引用可能破坏跨文档的完整性。策略包括：
    1. **目标URI冲突**：如果两个用户同时修改`target`字段，系统会保留所有版本并在UI中要求用户明确选择。
    2. **引用类型冲突**：`ref_type`的并发修改通常使用LWW规则，但会在metadata中记录冲突历史。
    3. **引用完整性检查**：解决冲突后，系统会异步验证新的目标URI是否可达，并在metadata中更新验证状态。
-   **`type: "recipe"`**：Recipe区块的冲突解决涉及复杂的配置合并：
    1. **选择器冲突**：多个用户修改同一Recipe的选择器时，系统尝试语义合并（如合并标签过滤条件）。
    2. **转换规则冲突**：转换管道的并发修改会保留所有版本，要求用户手动解决，因为自动合并可能产生无效的转换逻辑。
    3. **输出配置冲突**：输出格式等配置项使用LWW规则，但在metadata中保留冲突记录用于审计。
...
## 2.6. 通过元数据实现层级结构 (Hierarchical Structures via Metadata)
