...
mod performance_benchmarks {
    ...
    fn test_block_creation_performance() {
        ...
        for i in 0..BLOCK_COUNT {
            let block = Block::new(Uuid::new_v4().to_string(), "markdown".to_string())
                .with_name(format!("benchmark-block-{}", i))
                .with_content(BlockContent::Text(format!("Benchmark content {}", i)))
                .with_attributes({
                    let mut attrs = HashMap::new();
                    attrs.insert("index".to_string(), serde_json::json!(i));
                    attrs.insert("type".to_string(), serde_json::json!("benchmark"));
                    attrs
                });
            blocks.push(block);
        }
        ...
        println!("创建 {} 个Block耗时: {:?}", BLOCK_COUNT, creation_time);
        println!("平均每个Block创建时间: {:?}", creation_time / BLOCK_COUNT as u32);
        
        // 性能基准: 1000个Block应该在100ms内创建完成
        assert!(creation_time < Duration::from_millis(100), 
                "Block创建性能不达标，耗时: {:?}", creation_time);
    }

    /// 测试目标: Document序列化性能基准
    #[test]
    fn test_document_serialization_performance() {
        let mut doc = Document::new("serialization-benchmark".to_string());
        
        // 添加多种类型的Block
        for i in 0..200 {
            let block_type = match i % 4 {
                0 => "markdown",
                1 => "code", 
                2 => "relations",
                _ => "binary",
            ...
            let content = match block_type {
                "relations" => BlockContent::Relations(format!(
                    "block-{} -> block-{} [child_of] {{index: {}}}", 
                    i, (i + 1) % 200, i
                )),
                "binary" => BlockContent::Binary(vec![i as u8; 1000]), // 1KB per binary block
                _ => BlockContent::Text(format!("Content for block {} of type {}", i, block_type)),
            };
            
            let block = Block::new(Uuid::new_v4().to_string(), block_type.to_string())
                .with_name(format!("perf-block-{}", i))
                .with_content(content)
                .with_attributes({
                    let mut attrs = HashMap::new();
                    attrs.insert("index".to_string(), serde_json::json!(i));
                    attrs.insert("benchmark".to_string(), serde_json::json!(true));
                    attrs
                });
            doc.blocks.push(block);
        }
        ...
        println!("序列化包含200个Block的Document耗时: {:?}", serialize_time);
        println!("序列化结果大小: {} bytes", serialized.len());
        ...
        println!("反序列化Document耗时: {:?}", deserialize_time);
        ...
        // 性能基准
        assert!(serialize_time < Duration::from_millis(500), 
                "序列化性能不达标，耗时: {:?}", serialize_time);
        assert!(deserialize_time < Duration::from_millis(1000), 
                "反序列化性能不达标，耗时: {:?}", deserialize_time);
    }

    /// 测试目标: 复杂查找操作性能基准
    #[test]
    fn test_complex_search_performance() {
        let mut doc = Document::new("search-benchmark".to_string());
        
        // 创建具有复杂属性结构的Block
        for i in 0..1000 {
            let block = Block::new(Uuid::new_v4().to_string(), "markdown".to_string())
                .with_name(format!("search-block-{:04}", i))
                .with_content(BlockContent::Text(format!("Searchable content {}", i)))
                .with_attributes({
                    let mut attrs = HashMap::new();
                    attrs.insert("index".to_string(), serde_json::json!(i));
                    attrs.insert("category".to_string(), serde_json::json!(format!("cat-{}", i % 10)));
                    attrs.insert("priority".to_string(), serde_json::json!(i % 5));
                    attrs.insert("tags".to_string(), serde_json::json!(vec![
                        format!("tag-{}", i % 3),
                        format!("type-{}", i % 7),
                        format!("group-{}", i % 11)
                    ]));
        ...
        // 测试ID查找性能
        let id_search_start = Instant::now();
        let random_block = &doc.blocks[500];
        let found_by_id = doc.find_block(&random_block.id);
        let id_search_time = id_search_start.elapsed();
        
        assert!(found_by_id.is_some());
        println!("按ID查找耗时: {:?}", id_search_time);
        
        // 测试名称查找性能
        let name_search_start = Instant::now();
        let found_by_name = doc.find_block_by_name("search-block-0750");
        let name_search_time = name_search_start.elapsed();
        
        assert!(found_by_name.is_some());
        println!("按名称查找耗时: {:?}", name_search_time);
        
        // 测试复杂属性过滤性能
        let filter_start = Instant::now();
        let filtered_blocks: Vec<_> = doc.blocks.iter()
            .filter(|block| {
                block.attributes.get("category")
                    .and_then(|v| v.as_str())
                    .map(|s| s == "cat-5")
                    .unwrap_or(false)
                && block.attributes.get("priority")
                    .and_then(|v| v.as_i64())
                    .map(|p| p >= 3)
                    .unwrap_or(false)
            })
        ...
        println!("复杂属性过滤耗时: {:?}，结果: {} 个Block", filter_time, filtered_blocks.len());
        
        // 性能基准
        assert!(id_search_time < Duration::from_millis(10));
        assert!(name_search_time < Duration::from_millis(50));
    ...
    fn test_relation_performance() {
        const RELATION_COUNT: usize = 10000;
        ...
        for i in 0..RELATION_COUNT {
            let relation = Relation::new(
                format!("block-{}", i),
                format!("block-{}", (i + 1) % RELATION_COUNT),
                match i % 5 {
                    0 => "child_of".to_string(),
                ...
                let mut attrs = HashMap::new();
                attrs.insert("weight".to_string(), serde_json::json!(i as f64 / RELATION_COUNT as f64));
                attrs.insert("created_at".to_string(), serde_json::json!(i));
                attrs
            });
        ...
        println!("创建 {} 个Relation耗时: {:?}", RELATION_COUNT, creation_time);
        ...
        println!("验证 {} 个Relation耗时: {:?}", RELATION_COUNT, validation_time);
        
        // 测试Relation序列化性能
        let serialization_start = Instant::now();
        let mut serialized_relations = Vec::new();
        for relation in &relations[..100] { // 只序列化前100个
            let serialized = serde_json::to_string(relation).expect("序列化成功");
            serialized_relations.push(serialized);
        }
        let serialization_time = serialization_start.elapsed();
        println!("序列化 100 个Relation耗时: {:?}", serialization_time);
        
        // 性能基准
        assert!(creation_time < Duration::from_millis(500));
        assert!(validation_time < Duration::from_millis(200));
